...
const CreateCar = async (req, res) => {
    try {
        ...
        if (oldCar) {
            return res.status(400).json({ message: "Bunday model mavjud, oldin bu model ro'yhatdan o'tkazilgan" });
        }
        ...
        await newcar.save();
        res.status(200).json({ message: "Mashina muvaffaqiyatli qo'shildi", car: newcar });
    } catch (error) {
        console.log(error);
...
const updateCar = async (req, res) => {
    try {
        const { id } = req.params;
        const { model, description, color, horsePower, carType, charging, weight, gasoline, yearMachine, price } = req.body;
